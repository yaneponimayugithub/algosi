...
from lab1.task6.src.bubble_sort import bubble_sort, get_memory_usage
def pr(num):
    print('-'*30,num,"-"*30)
...
class MyTestCase(unittest.TestCase):
    ...
    def test1(self):
        ...
        t_start = time.perf_counter()
        initial_memory = get_memory_usage()
        self.assertEqual(bubble_sort(random_list),[-5, 1, 2, 3, 8, 123])
        final_memory = get_memory_usage()
        pr("test1")
        print(f"Время работы: {time.perf_counter() - t_start:.8f} секунд")
        print(f"Память: {final_memory - initial_memory:.8f} МБ \n")
